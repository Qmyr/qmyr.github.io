...
const CATE_EXCLUDE = '首页|留言|APP|下载|资讯|新闻|动态|福利|伦理';
...
let VOD = {};// 二级的单个数据
let TABS = [];// 二级的自定义线路列表 如: TABS=['在线播放','在线播放2']
 ...
 function init(ext) {
    ...
    try {
        ...
        if (typeof (globalThis.mubanJs) === 'undefined') {
            let mubanJs = request('./muban.js', { 'User-Agent': MOBILE_UA });
